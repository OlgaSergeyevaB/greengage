...
#include "pg_config.h"
	...
	#error "Cannot build autopart with PostgreSQL version lower than 9.5.3"
#elif PG_VERSION_NUM < 90504
	#warning "It is STRONGLY recommended to use autopart with PostgreSQL 9.5.4 since it contains important fixes"
#endif
...
#define BeginCopyFromCompat(pstate, rel, filename, is_program, data_source_cb, \
		...
		BeginCopyFrom((pstate), (rel), (filename), (is_program), \
					  (data_source_cb), NULL, (attnamelist), (options))
#elif PG_VERSION_NUM >= 90500
#define BeginCopyFromCompat(pstate, rel, filename, is_program, data_source_cb, \
							attnamelist, options) \
...
#define create_nestloop_path_compat(root, joinrel, jointype, orig_jointype, workspace, extra, \
									outer, inner, filtered_joinclauses, redistribution_clauses, pathkeys, \
									required_outer) \
		create_nestloop_path((root), (joinrel), (jointype), (orig_jointype), (workspace), (extra), \
							 (outer), (inner), (filtered_joinclauses), (redistribution_clauses), (pathkeys), \
							 (required_outer))
#elif PG_VERSION_NUM >= 90500
 ...
 */
#if PG_VERSION_NUM >= 100000
#define DefineRelationCompat(createstmt, relkind, ownerId, typaddress, intoPolicy) \
	DefineRelation((createstmt), (relkind), (ownerId), (typaddress), NULL, false, false, intoPolicy)
#elif PG_VERSION_NUM >= 90500
