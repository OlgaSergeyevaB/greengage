...
MemoryContext		TopAutopartContext				= NULL;
MemoryContext		AutopartParentsCacheContext		= NULL;
MemoryContext		AutopartStatusCacheContext		= NULL;
MemoryContext		AutopartBoundsCacheContext		= NULL;
...
/* autopart's init status */
AutopartInitState 	autopart_init_state;

/* autopart's hooks state */
bool				autopart_hooks_enabled = true;


/* Functions for various local caches */
static bool init_autopart_relation_oids(void);
static void fini_autopart_relation_oids(void);
...
/*
 * Safe hash search (takes care of disabled autopart).
 */
void *
autopart_cache_search_relid(HTAB *cache_table,
						   Oid relid,
						   HASHACTION action,
						   bool *found)
{
		...
		switch (action)
		{
			...
			default:
				elog(ERROR, "unexpected action in function "
					 CppAsString(autopart_cache_search_relid));
				break;
		}
...
void
save_autopart_init_state(AutopartInitState *temp_init_state)
{
	*temp_init_state = autopart_init_state;
}

void
restore_autopart_init_state(const AutopartInitState *temp_init_state)
{
 	/*
	 ...
	 */
	autopart_init_state.autopart_enable = temp_init_state->autopart_enable;
	autopart_init_state.auto_partition = temp_init_state->auto_partition;
	autopart_init_state.override_copy = temp_init_state->override_copy;
}
 ...
 */
void
init_main_autopart_toggles(void)
{
	/* Main toggle, load_config() will enable it */
	DefineCustomBoolVariable(AUTOPART_ENABLE,
							 "Enables autopart's optimizations during planning stage",
							 NULL,
							 &autopart_init_state.autopart_enable,
							 DEFAULT_AUTOPART_ENABLE,
							 PGC_SUSET,
							 0,
							 NULL,
							 autopart_enable_assign_hook,
							 NULL);

	/* Global toggle for automatic partition creation */
	DefineCustomBoolVariable(AUTOPART_ENABLE_AUTO_PARTITION,
							 "Enables automatic partition creation",
							 NULL,
							 &autopart_init_state.auto_partition,
							 DEFAULT_AUTOPART_AUTO,
							 PGC_SUSET,
							 0,
							 NULL,
							 NULL,
	...
	DefineCustomBoolVariable(AUTOPART_OVERRIDE_COPY,
							 "Override COPY statement handling",
							 NULL,
							 &autopart_init_state.override_copy,
							 DEFAULT_AUTOPART_OVERRIDE_COPY,
							 PGC_SUSET,
							 0,
							 NULL,
							 NULL,
...
/*
 * Create local PartRelationInfo cache & load autopart's config.
...
load_config(void)
{
	...
	/* Register autopart_relcache_hook(), currently we can't unregister it */
	if (relcache_callback_needed)
	{
		CacheRegisterRelcacheCallback(autopart_relcache_hook, PointerGetDatum(NULL));
		relcache_callback_needed = false;
	}

	/* Mark autopart as initialized */
	autopart_init_state.initialization_needed = false;

	elog(DEBUG2, "autopart's config has been loaded successfully [%u]", MyProcPid);
...
unload_config(void)
{
	/* Don't forget to reset autopart's cached relids */
	fini_autopart_relation_oids();

	/* Destroy 'partitioned_rels' & 'parent_cache' hash tables */
	fini_local_cache();

	/* Mark autopart as uninitialized */
	autopart_init_state.initialization_needed = true;

	elog(DEBUG2, "autopart's config has been unloaded successfully [%u]", MyProcPid);
}

/*
 * Estimate total amount of shmem needed for autopart to run.
 */
Size
estimate_autopart_shmem_size(void)
{
	return 0;
}

/*
 * Cache *all* important autopart's relids at once.
 * We should NOT rely on any previously cached values.
 */
static bool
init_autopart_relation_oids(void)
{
	Oid schema = get_autopart_schema();
	Assert(schema != InvalidOid);

	/* Cache AUTOPART_CONFIG relation's Oid */
	autopart_config_relid = get_relname_relid(AUTOPART_CONFIG, schema);
	if (autopart_config_relid == InvalidOid)
		return false;

	/* Cache AUTOPART_CONFIG_PARAMS relation's Oid */
	autopart_config_params_relid = get_relname_relid(AUTOPART_CONFIG_PARAMS,
													schema);
	if (autopart_config_params_relid == InvalidOid)
		return false;
...
/*
 * Forget *all* autopart's cached relids.
 */
static void
fini_autopart_relation_oids(void)
{
	autopart_config_relid = InvalidOid;
...
init_local_cache(void)
{
	...
	/* Reset autopart's memory contexts */
	if (TopAutopartContext)
	{
		/* Check that child contexts exist */
		Assert(MemoryContextIsValid(AutopartParentsCacheContext));
		Assert(MemoryContextIsValid(AutopartStatusCacheContext));
		Assert(MemoryContextIsValid(AutopartBoundsCacheContext));

		/* Clear children */
		MemoryContextReset(AutopartParentsCacheContext);
		MemoryContextReset(AutopartStatusCacheContext);
		MemoryContextReset(AutopartBoundsCacheContext);
	}
	/* Initialize autopart's memory contexts */
	else
	{
		Assert(AutopartParentsCacheContext == NULL);
		Assert(AutopartStatusCacheContext == NULL);
		Assert(AutopartBoundsCacheContext == NULL);

		TopAutopartContext =
				AllocSetContextCreate(TopMemoryContext,
									  AUTOPART_TOP_CONTEXT,
									  ALLOCSET_DEFAULT_SIZES);

		/* For PartParentInfo */
		AutopartParentsCacheContext =
				AllocSetContextCreate(TopAutopartContext,
									  AUTOPART_PARENTS_CACHE,
									  ALLOCSET_SMALL_SIZES);

		/* For PartStatusInfo */
		AutopartStatusCacheContext =
				AllocSetContextCreate(TopAutopartContext,
									  AUTOPART_STATUS_CACHE,
									  ALLOCSET_SMALL_SIZES);

		/* For PartBoundInfo */
		AutopartBoundsCacheContext =
				AllocSetContextCreate(TopAutopartContext,
									  AUTOPART_BOUNDS_CACHE,
									  ALLOCSET_SMALL_SIZES);
	}
	...
	ctl.hcxt = AutopartParentsCacheContext;

	parents_cache = hash_create(AUTOPART_PARENTS_CACHE,
								PART_RELS_SIZE, &ctl,
	...
	ctl.hcxt = AutopartStatusCacheContext;

	status_cache = hash_create(AUTOPART_STATUS_CACHE,
	...
	ctl.hcxt = AutopartBoundsCacheContext;

	bounds_cache = hash_create(AUTOPART_BOUNDS_CACHE,
							   PART_RELS_SIZE * CHILD_FACTOR, &ctl,
							   HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
}
...
build_check_constraint_name_relname_internal(const char *relname)
{
	...
	return psprintf("autopart_%s_check", relname);
}
...
/*
 * Check that relation 'relid' is partitioned by autopart.
 * Extract tuple into 'values', 'isnull', 'xmin', 'iptr' if they're provided.
 */
bool
autopart_config_contains_relation(Oid relid, Datum *values, bool *isnull,
								 TransactionId *xmin, ItemPointerData* iptr)
{
	...
	ScanKeyInit(&key[0],
				Anum_autopart_config_partrel,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(relid));

	/* Open AUTOPART_CONFIG with latest snapshot available */
	rel = heap_open_compat(get_autopart_config_relid(false), AccessShareLock);

	/* Check that 'partrel' column is of regclass type */
	Assert(TupleDescAttr(RelationGetDescr(rel),
				Anum_autopart_config_partrel - 1)->atttypid == REGCLASSOID);

	/* Check that number of columns == Natts_autopart_config */
	Assert(RelationGetDescr(rel)->natts == Natts_autopart_config);
	...
	while ((htup = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		...
		if (values && isnull)
		{
			...
			Assert(!isnull[Anum_autopart_config_partrel - 1]);
			Assert(!isnull[Anum_autopart_config_expr - 1]);
			Assert(!isnull[Anum_autopart_config_parttype - 1]);
		}
	...
	elog(DEBUG2, "AUTOPART_CONFIG %s relation %u",
...
/*
 * Loads additional autopart parameters like 'enable_parent'
 * or 'auto' from AUTOPART_CONFIG_PARAMS.
 */
bool
read_autopart_params(Oid relid, Datum *values, bool *isnull)
{
	Relation		rel;
	TableScanDesc	scan;
	ScanKeyData		key[1];
	...
	ScanKeyInit(&key[0],
				Anum_autopart_config_params_partrel,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(relid));

	rel = heap_open_compat(get_autopart_config_params_relid(false), AccessShareLock);
	snapshot = RegisterSnapshot(GetLatestSnapshot());
	scan = table_beginscan(rel, snapshot, 1, key);
	...
	if ((htup = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		...
		/* Perform checks for non-NULL columns */
		Assert(!isnull[Anum_autopart_config_params_partrel - 1]);
		Assert(!isnull[Anum_autopart_config_params_enable_parent - 1]);
		Assert(!isnull[Anum_autopart_config_params_auto - 1]);
		Assert(!isnull[Anum_autopart_config_params_spawn_using_bgw - 1]);
	}
...
version_error:
	DisableAutopart(); /* disable autopart since config is broken */
	ereport(ERROR, (errmsg("wrong version: \"%s\"", version_cstr),
					errhint(INIT_ERROR_HINT)));
	return 0; /* keep compiler happy */
}

/* Get version of autopart's facade written in Pl/PgSQL */
static uint32
get_plpgsql_frontend_version(void)
{
	...
	/* Extract autopart's version as cstring */
...
validate_plpgsql_frontend_version(uint32 current_ver, uint32 compatible_ver)
{
	...
	if (current_ver < compatible_ver)
	{
		...
		DisableAutopart(); /* disable autopart since config is broken */
		ereport(ERROR,
				(errmsg("autopart's Pl/PgSQL frontend is incompatible with "
						"its shared library"),
