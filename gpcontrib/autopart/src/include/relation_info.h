...
typedef struct PartRelationInfo
{
	...
	List		   *owners;			/* saved callers of get_autopart_relation_info() */
...
PrelLastChild(const PartRelationInfo *prel)
{
	if (PrelChildrenCount(prel) == 0)
		elog(ERROR, "autopart's cache entry for relation %u has 0 children",
...
/* Dispatch cache */
bool has_autopart_relation_info(Oid relid);
PartRelationInfo *get_autopart_relation_info(Oid relid);
void close_autopart_relation_info(PartRelationInfo *prel);
...
void delay_autopart_shutdown(void);
...
extern bool			autopart_enable_bounds_cache;
...
#define get_autopart_relation_info(relid) \
	( \
		prel_resowner_function = __FUNCTION__, \
		prel_resowner_line = __LINE__, \
		get_autopart_relation_info(relid) \
	)

#define close_autopart_relation_info(prel) \
	do { \
