...
static const struct config_enum_entry autopart_insert_into_fdw_options[] = {
...
bool				autopart_enable_partition_filter = true;
int					autopart_insert_into_fdw = PF_FDW_INSERT_POSTGRES;
...
init_partition_filter_static_data(void)
{
	...
	DefineCustomBoolVariable("autopart.enable_partitionfilter",
							 "Enables the planner's use of " INSERT_NODE_NAME " custom node.",
							 NULL,
							 &autopart_enable_partition_filter,
							 true,
							 PGC_USERSET,
							 0,
							 NULL,
	...
	DefineCustomEnumVariable("autopart.insert_into_fdw",
							 "Allow INSERTS into FDW partitions.",
							 NULL,
							 &autopart_insert_into_fdw,
							 PF_FDW_INSERT_POSTGRES,
							 autopart_insert_into_fdw_options,
							 PGC_SUSET,
							 0,
							 NULL,
							 NULL,
						  ...
						  void *fini_rri_holder_cb_arg)
{
	...
	parts_storage->prel = get_autopart_relation_info(parent_relid);
...
fini_result_parts_storage(ResultPartsStorage *parts_storage)
{
	...
	while ((rri_holder = (ResultRelInfoHolder *) hash_seq_search(&stat)) != NULL)
	{
		...
		if (rri_holder->prel)
			close_autopart_relation_info(rri_holder->prel);
	}
	...
	/* Don't forget to close 'prel'! */
	close_autopart_relation_info(parts_storage->prel);
}
...
#define CopyToResultRelInfo(field_name) \
	...
	if (!found)
	{
		...
		if ((rri_holder->prel = get_autopart_relation_info(partid)) != NULL)
		{
...
refresh_result_parts_storage(ResultPartsStorage *parts_storage, Oid partid)
{
	...
	{
		close_autopart_relation_info(parts_storage->prel);
		parts_storage->prel = get_autopart_relation_info(partid);
	...
	else
	{
		...
		close_autopart_relation_info(rri_holder->prel);
		rri_holder->prel = get_autopart_relation_info(partid);
...
/*
 * Convert a list of IndexRanges (rangeset) into an array of partition Oids.
 * (formerly lived in nodes_common.c, kept here for INSERT routing).
 */
 ...
 */
/*
 * Compute the [lower, upper) RANGE bounds (with origin 0) that a value falls
 * into, using the interval recorded in AUTOPART_CONFIG.  Only INT4 partition
 * keys with a positive integer interval are supported for now.
 */
static void
compute_range_bounds(Oid parent_relid, Oid value_type, Datum value,
					 int32 *lower, int32 *upper)
{
	...
	if (value_type != INT4OID)
		elog(ERROR, "auto partition creation currently supports only int4 RANGE keys");

	if (!autopart_config_contains_relation(parent_relid, values, isnull, NULL, NULL))
		elog(ERROR, "table \"%s\" is not partitioned",
			 get_rel_name_or_relid(parent_relid));

	if (isnull[Anum_autopart_config_range_interval - 1])
		elog(ERROR, "partition interval is NULL for relation \"%s\"",
			 get_rel_name_or_relid(parent_relid));

	interval_cstr = TextDatumGetCString(values[Anum_autopart_config_range_interval - 1]);
	interval = atoi(interval_cstr);
	if (interval <= 0)
		elog(ERROR, "invalid (non-positive) partition interval \"%s\"", interval_cstr);
...
/*
 * QE side: obtain the partition relation covering [lower, upper).
 *
 * If it already exists (created by a previous tuple/statement or another
 * session) we just route into it.  Otherwise we notify the coordinator, which
 * has a background worker create and attach the partition cluster-wide in its
 * own committed transaction.  Once the coordinator answers, the partition is
 * already present on this segment, so we refresh our catalog snapshot and look
 * it up by its deterministic name.
 */
static Oid
request_partition_from_coordinator(Oid parent_relid, int32 lower, int32 upper)
{
	char			payload[128];
	unsigned char	qtype;
	int				retval;
	...
	/* Already created by a previous statement / tuple? */
	relid = get_relname_relid(make_range_partition_name(parent_relid, lower, upper),
							  get_rel_namespace(parent_relid));
	if (OidIsValid(relid))
		return relid;

	/* Ask the coordinator to create the partition for this range. */
	snprintf(payload, sizeof(payload), "%u:%d:%d", parent_relid, lower, upper);
	NotifyMyFrontEnd("partition", payload, gp_session_id);
	pq_flush();

	/* Wait for the coordinator's response (mirrors cdb_sequence_nextval_qe). */
	do
	{
		pq_startmsgread();
		...
		if (retval == EOF)
			ereport(ERROR,
					(errcode(ERRCODE_INTERNAL_ERROR),
					 errmsg("partition request: connection is gone unexpectedly")));
	} while (retval != 1);

	if (qtype == 'X')
		ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
						errmsg("partition request: QD closed the connection")));
	if (qtype != SEQ_NEXTVAL_QUERY_RESPONSE)
		ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
						errmsg("partition request: unexpected message type='%c'", qtype)));

	initStringInfo(&buf);
	if (pq_getmessage(&buf, 0) != 0)
		elog(ERROR, "partition request: unable to read response from QD");

	/* First byte is the error flag. */
	error = (buf.len > 0 && buf.data[0] == SEQ_NEXTVAL_TRUE);
	if (error)
	{
		pfree(buf.data);
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("coordinator failed to create partition for range [%d, %d)",
						lower, upper)));
	}
	...
	{
		/*
		 * Heavy-lock path: the coordinator could not use a worker and shipped
		 * the preassigned OIDs.  Recreate an identical relation locally and
		 * remember it; the coordinator attaches it cluster-wide at end of
		 * statement (attach_created_partitions()).
		 */
		List   *oids = (List *) deserializeNode(buf.data + 1, buf.len - 1);

		AddPreassignedOids(oids);
		relid = create_partition_relation_local(parent_relid,
												make_range_partition_name(parent_relid,
																		  lower, upper));
		remember_segment_partition(parent_relid, lower, upper, relid);
		pfree(buf.data);

		elog(DEBUG1, "autopart[QE seg%d]: created partition relid=%u [%d,%d) noids=%d",
			 GpIdentity.segindex, relid, lower, upper, list_length(oids));
		return relid;
	}

	/*
	 * Worker path: the partition was already created and attached cluster-wide
	 * by a background worker.  Refresh our catalog snapshot and look it up.
	 */
	pfree(buf.data);
	...
	if (!OidIsValid(relid))
		elog(ERROR,
			 "autopart: partition for range [%d, %d) not found after creation",
			 lower, upper);

	elog(DEBUG1, "autopart[QE seg%d]: routed into partition relid=%u [%d,%d)",
		 GpIdentity.segindex, relid, lower, upper);
							...
							TupleTableSlot *slot)
{
	...
	do
	{
		...
		{
			/*
			 * No partition holds this value.  On a segment we ask the
			 * coordinator to allocate a new partition for the value's range
			 * and (re)create it locally; the coordinator does the same on its
			 * own catalog.  Data tuples only ever flow on the segments, so the
			 * dispatcher should not reach this branch.
			 */
			int32	lower,
					upper;
			...
			compute_range_bounds(parent_relid, prel->ev_type, value,
								 &lower, &upper);

			partition_relid = request_partition_from_coordinator(parent_relid,
																 lower, upper);
		}
					  ...
					  List *returning_list)
{
	...
	cscan->custom_private = list_make4(makeConst(INT4OID,
										-1,
										InvalidOid,
										sizeof(int32),
										Int32GetDatum(parent_relid),
										false,
										true),
										makeConst(INT4OID,
										-1,
...
partition_filter_create_scan_state(CustomScan *node)
{
	...
	state->partitioned_table	=  DatumGetInt32(((Const *)(linitial(node->custom_private)))->constvalue);
	state->on_conflict_action	= DatumGetInt32(((Const *)(lsecond(node->custom_private)))->constvalue);
	state->returning_list		= (List *) lthird(node->custom_private);
	state->command_type			=(CmdType)DatumGetInt32(((Const *)(lfourth(node->custom_private)))->constvalue);
...
partition_filter_exec(CustomScanState *node)
{
	...
	if (!TupIsNull(slot))
	{
		...
		if (!estate->es_tuple_routing_result_relations)
...
partition_filter_end(CustomScanState *node)
{
	...
	/*
	 * Worker path: nothing to do (the worker already created+attached).
	 * Heavy-lock path: the coordinator dispatches CREATE (if_not_exists, with
	 * the saved OIDs) for every on-demand partition to all segments and attaches
	 * them now; segments only clear their bookkeeping.  No-op if the list is
	 * empty (i.e. only the worker path was used).
	 */
						   ...
						   const ResultPartsStorage *rps_storage)
{
	...
	/* Perform some checks according to 'autopart_insert_into_fdw' */
	switch (autopart_insert_into_fdw)
	{
