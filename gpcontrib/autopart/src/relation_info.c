...
#undef get_autopart_relation_info
#undef close_autopart_relation_info
...
/*
 * For autopart.enable_bounds_cache GUC.
 */
bool			autopart_enable_bounds_cache = true;


/*
 * We delay all invalidation jobs received in relcache hook.
 */
static bool		delayed_shutdown = false; /* autopart was dropped */
...
static PartRelationInfo *build_autopart_relation_info(Oid relid, Datum *values);
static void free_autopart_relation_info(PartRelationInfo *prel);
...
{
	DefineCustomBoolVariable("autopart.enable_bounds_cache",
							 "Make updates of partition dispatch cache faster",
							 NULL,
							 &autopart_enable_bounds_cache,
							 true,
							 PGC_USERSET,
							 0,
							 NULL,
...
forget_status_of_relation(Oid relid)
{
	...
	psin = autopart_cache_search_relid(status_cache,
									  relid, HASH_FIND,
									  NULL);
	if (psin)
	 ...
	 */
	ppar = autopart_cache_search_relid(parents_cache,
									  relid, HASH_FIND,
									  NULL);
...
invalidate_psin_entry(PartStatusInfo *psin)
{
	...
	if (psin->prel)
	{
		...
		{
			free_autopart_relation_info(psin->prel);
		}
	}

	(void) autopart_cache_search_relid(status_cache,
									  psin->relid,
									  HASH_REMOVE,
									  NULL);
}
...
void
close_autopart_relation_info(PartRelationInfo *prel)
{
	AssertArg(prel);
...
/* Check if relation is partitioned by autopart */
bool
has_autopart_relation_info(Oid relid)
{
	PartRelationInfo *prel;

	if ((prel = get_autopart_relation_info(relid)) != NULL)
	{
		close_autopart_relation_info(prel);
...
PartRelationInfo *
get_autopart_relation_info(Oid relid)
{
	PartStatusInfo *psin;
	...
	if (!psin)
	{
		...
		/*
		 * Check if AUTOPART_CONFIG table contains this relation and
		 * build a partitioned table cache entry (might emit ERROR).
		 */
		if (autopart_config_contains_relation(relid, values, isnull, NULL, &iptr))
			prel = build_autopart_relation_info(relid, values);
...
build_autopart_relation_info(Oid relid, Datum *values)
{
	...
	prel_mcxt = AllocSetContextCreate(AutopartParentsCacheContext,
									  "build_autopart_relation_info",
	...
	PG_TRY();
	{
		...
		Datum					param_values[Natts_autopart_config_params];
		bool					param_isnull[Natts_autopart_config_params];
		Oid					   *prel_children;
		...
		prel->parttype	= DatumGetPartType(values[Anum_autopart_config_parttype - 1]);

		/* Switch to persistent memory context */
		old_mcxt = MemoryContextSwitchTo(prel->mcxt);

		/* Build partitioning expression tree */
		prel->expr_cstr = TextDatumGetCString(values[Anum_autopart_config_expr - 1]);
		...
		if (read_autopart_params(relid, param_values, param_isnull))
		{
			prel->enable_parent =
					param_values[Anum_autopart_config_params_enable_parent - 1];
		}
		/* Else set default values if they cannot be found */
		else
		{
			prel->enable_parent = DEFAULT_AUTOPART_ENABLE_PARENT;
		}
	}
	PG_CATCH();
	{
		...
		/* Free this entry */
		free_autopart_relation_info(prel);
...
static void
free_autopart_relation_info(PartRelationInfo *prel)
{
...
resowner_prel_add(PartRelationInfo *prel)
{
	...
	if (prel)
	{
		...
		/* Register this 'prel' */
		old_mcxt = MemoryContextSwitchTo(TopAutopartContext);
...
#define AllocZeroArray(part_type, context, elem_num, elem_type) \
		...
		switch (prel->parttype)
		{
			case PT_HASH:
				...
				if (pbin->part_idx >= PrelChildrenCount(prel))
				{
					...
					ereport(ERROR, (errmsg("autopart's cache for relation %d "
	...
	if (prel->parttype == PT_HASH)
		for (i = 0; i < PrelChildrenCount(prel); i++)
		{
			...
			{
				DisableAutopart(); /* disable autopart since config is broken */
				ereport(ERROR, (errmsg("autopart's cache for relation \"%s\" "
									   "has not been properly initialized",
...
forget_bounds_of_partition(Oid partition)
{
	...
	pbin = autopart_enable_bounds_cache ?
				autopart_cache_search_relid(bounds_cache,
										   partition,
										   HASH_FIND,
										   NULL) :
...
forget_bounds_of_rel(Oid relid)
{
	 ...
	 * cache or digging autopart_config on each relcache invalidation.
	 */
...
/*
 * Fill 'pbin' with a RANGE partition's bounds taken from its native
 * declarative partition descriptor (pg_class.relpartbound), rather than from
 * a autopart CHECK constraint.  This lets autopart route into ordinary
 * declarative partitions, including the ones auto-created on INSERT.
 */
static bool
fill_pbin_with_native_bounds(PartBoundInfo *pbin,
							 const PartRelationInfo *prel,
							 Oid partition)
{
	HeapTuple			tuple;
	...
	PartitionRangeDatum *lod,
						*hid;
	MemoryContext		old_mcxt;

	/* Only declarative RANGE partitions are handled natively here. */
	if (prel->parttype != PT_RANGE)
		return false;

	tuple = SearchSysCache1(RELOID, ObjectIdGetDatum(partition));
	if (!HeapTupleIsValid(tuple))
		elog(ERROR, "cache lookup failed for relation %u", partition);

	boundDatum = SysCacheGetAttr(RELOID, tuple,
								 Anum_pg_class_relpartbound, &isnull);
	if (isnull)
	{
		/* Not a native partition: fall back to the CHECK-constraint path. */
	...
	spec = (PartitionBoundSpec *) stringToNode(TextDatumGetCString(boundDatum));
	ReleaseSysCache(tuple);

	Assert(IsA(spec, PartitionBoundSpec));

	lod = (PartitionRangeDatum *) linitial(spec->lowerdatums);
	hid = (PartitionRangeDatum *) linitial(spec->upperdatums);

	old_mcxt = MemoryContextSwitchTo(AutopartBoundsCacheContext);

	if (lod->kind == PARTITION_RANGE_DATUM_MINVALUE)
		pbin->range_min = MakeBoundInf(MINUS_INFINITY);
	else
		pbin->range_min = MakeBound(datumCopy(((Const *) lod->value)->constvalue,
											  prel->ev_byval, prel->ev_len));

	if (hid->kind == PARTITION_RANGE_DATUM_MAXVALUE)
		pbin->range_max = MakeBoundInf(PLUS_INFINITY);
	else
		pbin->range_max = MakeBound(datumCopy(((Const *) hid->value)->constvalue,
											  prel->ev_byval, prel->ev_len));
...
get_bounds_of_partition(Oid partition, const PartRelationInfo *prel)
{
	...
	if (!pbin)
	{
		...
		/*
		 * Declarative (native) partitioning: read the partition's bounds
		 * directly from its relpartbound instead of a autopart CHECK
		 * constraint (which auto-created native partitions do not have).
		 * Fall back to the CHECK-constraint path for non-native partitions.
		 */
		if (!fill_pbin_with_native_bounds(&pbin_local, prel, partition))
		{
			Expr *con_expr = get_partition_constraint_expr(partition, true);
...
{
	autopart_cache_search_relid(parents_cache,
							   partition,
							   HASH_REMOVE,
							   NULL);
}
...
get_parent_of_partition(Oid partition)
{
	...
	else
	{
		...
		while ((htup = systable_getnext(scan)) != NULL)
		{
			...
			/* Check that AUTOPART_CONFIG contains this table */
			if (autopart_config_contains_relation(inhparent, NULL, NULL, NULL, NULL))
			{
							 ...
							 Oid *expr_type_out) /* ret value #1 */
{
	...
	/* We don't need autopart's magic here */
	autopart_hooks_enabled = false;
	...
	PG_CATCH();
	{
		...
		/* Don't forget to enable autopart's hooks */
		autopart_hooks_enabled = true;
...
/* Add new delayed autopart shutdown job (DROP EXTENSION) */
void
delay_autopart_shutdown(void)
{
...
finish_delayed_invalidation(void)
{
	...
	if (IsTransactionState())
	{
		...
		if (delayed_shutdown)
		{
			Oid cur_autopart_config_relid;

			/* Unset 'shutdown' flag */
			delayed_shutdown = false;

			/* Get current AUTOPART_CONFIG relid */
			cur_autopart_config_relid = get_relname_relid(AUTOPART_CONFIG,
														 get_autopart_schema());

			/* Check that AUTOPART_CONFIG table has indeed been dropped */
			if (cur_autopart_config_relid == InvalidOid ||
				cur_autopart_config_relid != get_autopart_config_relid(true))
			{
				/* Ok, let's unload autopart's config */
				unload_config();
